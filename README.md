...
```bash
npx ai-workflow-agent-skills install --agents claude --skills organize-docs,frame-opportunity,research-opportunity,stress-test-opportunity,plan-roadmap
```
...
```bash
DEST=~/.agents/skills
mkdir -p "$DEST" && cp -R skills/organize-docs skills/frame-opportunity skills/research-opportunity skills/stress-test-opportunity skills/plan-roadmap "$DEST/"
```
